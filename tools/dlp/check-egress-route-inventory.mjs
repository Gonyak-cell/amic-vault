...
export const EGRESS_INVENTORY = Object.freeze({
  ...
  'AmicOsVaultDocumentCopyService.readAndVerifySnapshot': {
    path: 'apps/api/src/modules/integrations/amic-os-vault-provider/amic-os-vault-document-copy.service.ts',
    category: 'internal_processing',
    rationale: 'reads one private retained copy only after the caller rechecks source access and exact version',
    authorityControl: 'tenant quarantine URI, stored size, MIME type and SHA-256 must match the retained copy',
    auditControl: 'download records documentDownloadedAudit before returning bytes',
    required: [
      'this.storageService.quarantineStorageUri(principal.tenantId, row.quarantine_ref)',
      'this.storageService.getByStorageUri(',
      'object.contentLength !== Number(row.byte_size)',
      'object.contentType !== row.mime_type',
      'readBounded(',
      'sha256(bytes) !== row.sha256',
    ],
  },
  'AmicOsVaultReadService.emailMessageForRow': {
    path: 'apps/api/src/modules/integrations/amic-os-vault-provider/amic-os-vault-read.service.ts',
    category: 'internal_processing',
    rationale: 'reads a bounded immutable EML header for an exact search projection only',
    authorityControl: 'tenant-scoped storage read follows current Matter, version and document permission filtering',
    auditControl: 'the authorized search operation retains its existing audit and exposes only parsed header fields',
    required: [
      'row.email_storage_uri && emailSourceForRow(row)',
      'this.storageService.getByStorageUri(principal.tenantId, row.email_storage_uri)',
      'readEmailHeaderPrefix(stored.body)',
