...
export function validateLockRow(row, sourceRoot) {
  ...
  for (const key of ['release', 'commit', 'tree', 'licensePath', 'licenseHash', 'clonePath', 'owner', 'state']) {
  ...
  if (row.state === 'BLOCKED') assert(typeof row.blockedReason === 'string' && row.blockedReason.trim(), `${row.id}: blocked reason missing`);
...
export function validateUpstreamLock({ map, sourceRoot, repoRoot = process.cwd() }) {
