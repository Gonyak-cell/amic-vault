...
export function validateTestReuse({ manifest, sourceMap, repoRoot = process.cwd(), exists = existsSync }) {
  ...
  for (const entry of manifest.entries) {
    ...
    if (entry.fixturePolicy === 'COPY') {
      assert(entry.classification === 'FIXTURE_REUSE' && DIGEST.test(entry.fixtureLicenseHash ?? '') && SHA.test(entry.fixtureBlob ?? ''), `${entry.id}: copied fixture lacks L2 provenance`);
    }
  ...
  assert(covered.has('OSS-04') && covered.has('OSS-05') && covered.has('OSS-06'), 'security-critical portfolio coverage missing');
