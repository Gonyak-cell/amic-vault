...
## Current PR-6C Acceptance

- `enterprise_dms_search_refiners` remains tenant-scoped, RLS/FORCE RLS-protected, admin-only for mutations, and audited with `refiner_id`, `field_key`, and status-only metadata.
- Search refiner `field_key` values are bounded to the fields implemented by the permission-scoped search UI/query contract; unsupported fields fail closed at shared schema and database constraint boundaries.
- `/v1/enterprise/dms/search-refiners/approved` exposes active search refiner contracts to internal users without raw `refiner_id` values.
- `/search` loads the approved refiner catalog and renders/requests only allowed filters and facets; direct URL or saved-search filters outside the approved catalog are stripped before search requests.
- `pnpm ui:production-smoke` guards approved refiner catalog consumption and request scrubbing.

## Current PR-6D Acceptance

- Outlook filing uses Matter Code suggestions from the hash-only, permission-scoped Matter suggestion endpoint; wall-excluded and cross-tenant Matters remain non-discoverable.
- Manual Outlook filing queues through `PermissionService.canUploadToMatter`, records `OUTLOOK_EMAIL_FILE_REQUESTED` or denied audit events with hash/ref/count metadata only, and status lookup rechecks same-user plus current Matter read permission.
- The Outlook task pane renders Matter Code safe labels and does not render raw mailbox address, raw Outlook id, email body, attachment filename, or raw Matter references as primary labels.
- Matter detail renders filed-email document links only from the permission-scoped Matter email timeline response and labels them as safe document actions rather than raw document IDs.
- `docs/release/enterprise-dms-ui-release-evidence.md`, `pnpm outlook:verification:check`, and `pnpm ui:production-smoke` guard DMS-GA-604 Outlook evidence. Authenticated Microsoft 365 tenant filing smoke remains an external reference-only receipt.

## Current PR-6E Acceptance

- `docs/adr/ADR-017-office-onedrive-flow.md` records Office/OneDrive as a planning gate only, not a connected feature.
- `/integrations/onedrive` remains `hidden_until_api_ready` with `showInNavigation: false` in `apps/web/src/lib/features.ts`.
- The integrations parent route may show OneDrive/Office only as gated states and must not link to a connected OneDrive route or claim open/save, coauthoring, live edit, lock, or sync.
- `docs/release/rollback-runbook.md` has an Office/OneDrive gate rollback path covering route hiding, callback/job rejection, consent/token disablement, integrity verification, and no hard delete.
- `pnpm ui:production-smoke` guards ADR-017, the route policy, the gated integration copy, and the rollback/evidence boundary. Follow-up runtime implementation still requires approved auth/storage/version/audit/callback/rollback contracts.

## Current PR-7A Acceptance

- `docs/release/enterprise-dms-monitor-map.md` defines DMS-MON-001 through DMS-MON-008 with opaque external query refs, owners, thresholds, aggregate dimensions, and rollback triggers.
- DMS-GA-305 reindex evidence is tied to admin-only route metadata, reference/count-only `SEARCH_REINDEX_REQUESTED` audit, UI queue/hash-only status, and monitor refs for reindex queue age/failure.
- `docs/release/enterprise-dms-ui-release-evidence.md` no longer leaves DMS-MON evidence refs blank; it points each row to `MON-DMS-*` refs and keeps DMS-UX-812 at `HOLD` until external monitor receipts are attached.
- `pnpm release:prod-preflight` and `pnpm ui:production-smoke` guard the monitor map and reindex evidence package. External monitoring review remains a required release blocker before production PASS.

## Current PR-7B Acceptance

- `docs/release/rollback-runbook.md` now defines the DMS-GA-702 rollback drill with `DMS-RB-001` through `DMS-RB-007`, `RB-DMS-*` repository evidence refs, `MON-DMS-*` trigger refs, named rollback owner requirements, and incident communication closure.
- Route visibility, Matter source flags, worker flags, database/audit invariants, storage integrity, monitor triggers, and Office/OneDrive gate rollback are documented as separate controls without hard delete or audit mutation.
- `docs/release/enterprise-dms-ui-release-evidence.md`, `docs/ui/enterprise-dms-release-hardening.md`, and `docs/release/production-ui-rollout-checklist.md` require external rollback drill refs before DMS-UX-812 can pass.
- `pnpm release:prod-preflight` and `pnpm ui:production-smoke` guard the DMS-GA-702 rollback package. External rollback rehearsal, rollback owner signoff, and production release approval remain separate release blockers.

## Current PR-7C Acceptance

- `docs/release/enterprise-dms-responsive-a11y-matrix.md` defines the DMS-GA-703 route-level receipt matrix for `DMS-RA-001` through `DMS-RA-007` and repository guards `RA-DMS-GUARD-001` through `RA-DMS-GUARD-004`.
- `docs/release/enterprise-dms-ui-release-evidence.md`, `docs/ui/enterprise-dms-release-hardening.md`, `docs/ui/enterprise-dms-pr-f-readiness.md`, and `docs/release/production-ui-rollout-checklist.md` require 1440px, 768px, 375px, keyboard, focus, accessible-name, `aria-current`, and screen-reader basics refs before DMS-UX-812 can pass.
- `pnpm ui:production-smoke` and `pnpm release:prod-preflight` guard the responsive/accessibility matrix and checklist refs. External visual QA and keyboard/screen-reader receipts remain required release blockers before production PASS.

## Current PR-7D Acceptance

- `tools/quality/check-production-ui-literals.mjs` now requires the DMS-GA-704 expanded upload, files, matter/team, search, records, audit, walls, work, notifications, admin, enterprise, integrations, Outlook, and AI Prep surfaces to remain in production literal scanning.
- `tools/quality/check-production-ui-literals.mjs` and `tools/release/check-production-ui-smoke.mjs` reject fake/mock/sample/demo operating copy, workspace ID, tenant ID, document ID, raw UUID literals, unsafe ID slices, ID-derived short hashes, legal analysis, document summary, external model, raw prompt/source, source text, and model-response copy.
- `tools/release/check-production-ui-smoke.mjs` rechecks the same expanded route set with `GUARD-DMS-001-SURFACE-COVERAGE` through `GUARD-DMS-004-AI-SCOPE-EXCLUSION` evidence refs documented in the release package.
- `docs/release/enterprise-dms-ui-release-evidence.md`, `docs/ui/enterprise-dms-release-hardening.md`, `docs/ui/enterprise-dms-pr-f-readiness.md`, and `docs/release/production-ui-rollout-checklist.md` keep these guards as repo-side gates only. External authenticated main-loop, negative-auth, responsive/accessibility receipts, and owner signoff remain release blockers before production PASS.

## Current PR-7E Acceptance

- `docs/release/enterprise-dms-ui-release-evidence.md` now defines the DMS-GA-705 signoff completion gate with `DMS-SIGNOFF-OPERATOR-REF`, `DMS-SIGNOFF-SECURITY-REF`, `DMS-SIGNOFF-LEGAL-DATA-REF`, `DMS-SIGNOFF-CUSTOMER-SCOPE-REF`, `DMS-SIGNOFF-ROLLBACK-REF`, exact scope, tenant scope, rollback owner, timestamp, and evidence package refs.
- `docs/release/production-ui-rollout-checklist.md` adds `UI-SIGNOFF-001` through `UI-SIGNOFF-006` and keeps the decision record at `HOLD` until external owner refs, exact production scope, exclusions, approved tenant class, rollback owner, release timestamp, and evidence package refs are attached.
- `docs/ui/enterprise-dms-release-hardening.md` and `docs/ui/enterprise-dms-pr-f-readiness.md` make missing DMS-GA-705 signoff refs explicit release blockers for DMS-UX-812.
- `pnpm ui:production-smoke` and `pnpm release:prod-preflight` guard the signoff package shape. They do not approve production release; external owner signoff and approved staging/canary DMS smoke receipts remain required before production PASS.
