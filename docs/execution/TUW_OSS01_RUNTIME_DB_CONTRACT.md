...
- **Files NOT-modify:** runtime source, env files, migrations, dependencies,
  lockfiles, workflows, `docs/package/**`.
- **Implementation:** reuse an existing parser if present; otherwise use a
  ...
  `QUE`). Runtime and CLI paths are distinct. The report fails on an
  unclassified construction or a new runtime direct constructor outside its
  registered batch.
