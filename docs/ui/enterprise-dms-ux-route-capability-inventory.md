...
| `/matters` | Matter app-backed matter list and workspace entry. | Partial. Vault matter list exists and each real row links to the Matter workspace, Matter Code filtered file cabinet, and Matter Code filtered search without fake counts or raw refs as visible labels. Vault-side Matter app source-of-truth guard now requires configured source plus runtime-ready lookup/sync before upload can become authoritative. | DMS-UX-801 |
...
| `/integrations/matter-app` | Matter app source/gate status. | Partial. Shows Matter Code source mode, upload-authoritative gate, lookup/sync runtime readiness, projection fallback policy, and setup-required state without exposing endpoints, tokens, internal Matter IDs, or connected-state claims before configuration. Runtime Matter app endpoint remains a release blocker until approved credentials and smoke receipts exist. | DMS-UX-003, 801 |
...
| --- | --- | --- | --- |
| Matter create/list/get/update/status/legal hold | `apps/api/src/modules/matter/*` | Exists as Vault matter API/projection. Vault upload/source UI treats it as non-authoritative unless Matter app source is configured and runtime-ready; production activation still needs authenticated smoke receipts. | DMS-UX-801 |
...
| Matter suggestions | `POST /search/matter-suggestions` | Exists for Outlook hash-only suggestion use. Normal Vault upload/search picker uses Matter Code/name/client labels and rejects Vault-internal references; production activation still requires runtime-ready Matter app source receipts. | DMS-UX-801 |
...
| Matter app lookup/sync | Matter app runtime endpoint not found in this checkout. | Gap/blocker for production-grade Matter Code activation. `/integrations/matter-app` now exposes safe configured/runtime-ready source gates so operators can see why upload is blocked without leaking endpoint or internal refs; descriptor-only Matter packages do not satisfy the runtime-ready flag. | DMS-UX-801 |
