...
| `/search` | Permission-bound enterprise search. | Partial. Backend title/body FTS, snippets, facets, audit, target scope, display-safe title/Matter Code/client filters, sort, URL state, display-safe grouping, current-search reusable link surface, and result actions to document detail/preview/file-cabinet filters exist. Search result detail links pass bounded hit context without URL-storing snippets/search terms. OCR/extraction, retention, persisted saved searches/search folders, and true preview anchor highlighting remain incomplete. | DMS-UX-308 to 314, 503, 609 |
