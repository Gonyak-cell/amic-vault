...
| `/files` | File upload and browse surface. | Partial. All-documents vault, server-backed document filters/sort, extraction/OCR status filters, Matter Code picker, upload panel, and matter-scoped browse surface exist with user-facing source labels; production navigation remains gated until Matter app source is configured and authenticated smoke is ready. | DMS-UX-102, 108, 111 |
| `/documents/[id]` | Document detail/action center. | Partial. Action center exists with profile read/edit, preview, controlled download reason, version list, add-version flow, governance context, records/audit links, and real-status work queue. Check-out/Office live edit remains deferred by ADR-016; contextual audit event feed and related-item APIs remain incomplete. | DMS-UX-406, 407, 501, 606 |
| `/search` | Permission-bound enterprise search. | Partial. Backend title/body FTS, snippets, facets, audit, target scope, display-safe title/Matter Code/client filters, sort, URL state, display-safe grouping, current-search reusable link surface, and result actions to document detail/preview/file-cabinet filters exist. OCR/extraction, retention, persisted saved searches/search folders, and in-preview hit controls remain incomplete. | DMS-UX-308 to 314, 503, 609 |
| `/records` | Records lifecycle console. | Partial/strong backend. Matter/document pages now link records context; records route still needs picker cleanup and disposal-task workflow integration. | DMS-UX-505 |
...
| `/admin` | Enterprise/admin settings. | Partial. SSO/BYOK/SIEM/backup/compliance surfaces exist; DMS taxonomy/template/refiner IA is visible as read-only contract state until save/audit APIs are approved. | DMS-UX-601 to 610 |
...
| Preview | Preview module exists in repo. | Document action center embeds the preview endpoint with unavailable/error handled by the server response; search results link directly to preview, but hit highlighting and next/previous hit navigation remain incomplete. | DMS-UX-308 |
