...
| `/admin` | Admin | `visible_admin_only` | Shown when role policy allows | Firm admin, security admin | Guarded admin settings route | SSO/MFA/BYOK/SIEM/Backup/Compliance data only after API success; taxonomy/template/refiner IA is read-only until save/audit APIs are approved; search index reprocessing uses the admin reindex endpoint and search health displays only bounded index/extraction/OCR/no-result audit aggregates; operations health uses local file organization prep health/metrics only |
