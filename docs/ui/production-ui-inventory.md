...
| `/search` | Vault | `visible` | Shown | Internal production users | AppShell route | Permission-before-search; no ID fallback; supports title/body/all target, display-safe Matter Code/client/title filters, URL state, sort, grouping, and current-search reusable links; OCR/searchability and persisted saved-search/search-folder APIs remain explicitly deferred |
