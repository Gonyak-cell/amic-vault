...
| `/search` | Vault | `visible` | Shown | Internal production users | AppShell route | Permission-before-search; no ID fallback; supports title/body/all target, display-safe Matter Code/client/title filters, URL state, sort, grouping, user-scoped saved searches, current-search reusable links, and result actions to document detail/preview/file-cabinet filters with bounded hit context; search-folder IA remains explicitly deferred |
