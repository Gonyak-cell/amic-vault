...
| `/files` | Vault | `visible` | Shown | Internal production users | AppShell route with all-documents vault, Matter Code picker, upload panel, and matter-scoped list foundation | Permission-scoped real document data only; no document ID or placeholder data; upload/list UX must stay Matter Code-gated and fail closed when Matter app source is unconfigured; source labels use operating-language copy, not implementation labels such as source-of-truth or projection |
| `/work` | Vault | `visible` | Shown | Internal production users | AppShell route | Work queue derives only from dashboard API sections and connection states; no persisted task counts or fake assignments before unified task API approval |
