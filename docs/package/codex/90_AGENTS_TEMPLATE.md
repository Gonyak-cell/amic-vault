...
- **Risk=C(Critical) TUW가 포함된 PR은 85번 §2-A의 기술 검증 체크리스트를 완료하고 결과를 PR에 남겨라.** 권한·격리·감사 negative test와 CI가 통과해야 한다. 운영자가 해당 PR의 병합을 명시적으로 지시한 경우 Codex가 최신 main·PR 상태를 재확인하고 병합할 수 있다. 외부 검토자를 별도로 요청한 PR에만 `needs-human-review` 라벨을 붙이고, 그 검토가 끝나기 전에는 병합하지 않는다.
