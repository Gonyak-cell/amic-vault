...
## H5 Basic Alert Binding

H5 keeps the repository side limited to safe aggregate metrics and external-ref
alarm contracts. `/metrics` exposes fixed-bucket HTTP histograms plus bounded
`pgboss_queue_depth` and `pgboss_dead_letter_jobs` gauges for `extraction`,
`indexing`, and `ai-prep`; it must not expose tenant, user, document, raw path,
object key, prompt, or source text labels.

| Alert ref                       | Signal                                             | Required source                                                                                                       | Owner    | Threshold                                                                                 | Operator response                                                                                                                       |
| ------------------------------- | -------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | -------- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `MON-H5-001-ALB-5XX-RATIO`      | ALB 5xx ratio spike                                | CloudWatch ALB target-group metric or approved equivalent external query ref                                          | Operator | Two consecutive windows above the approved launch baseline or any deploy-correlated spike | Pause traffic widening, preserve release refs, and roll back API/web image if the spike persists after queue drain checks               |
| `MON-H5-002-PGBOSS-QUEUE-DEPTH` | Queue backlog for extraction, indexing, or ai-prep | `/metrics` `pgboss_queue_depth` and `pgboss_dead_letter_jobs` gauges or CloudWatch metric transform from those gauges | Operator | Queue depth or dead-letter count exceeds the release-approved SLA for two windows         | Stop new enqueue paths for the affected worker class, inspect worker health, and keep user-facing receipts in pending/unavailable state |
| `MON-H5-003-ECS-WORKER-TASKS`   | Worker task count below desired count              | CloudWatch ECS service desired/running task count or approved equivalent external query ref                           | Operator | Running task count below desired count for one full evaluation window                     | Disable the affected worker flag when available, preserve queued jobs, and restart or roll back the worker service                      |

SNS/email evidence stays outside git. Release signoff requires an owner-reviewed
receipt proving each `MON-H5-*` ref is bound to the approved SNS topic or email
subscription without committing ARNs, account IDs, recipient addresses, tokens,
or production URLs.

## H6 Worker Process Binding

The API image now exposes separate runtime targets for request handling and
queue processing: `api` runs `apps/api/dist/main.js` with `PROCESS_ROLE=api`,
while `worker` runs `apps/api/dist/worker-main.js` with `PROCESS_ROLE=worker`.
H4 IaC must model this as a distinct ECS service or equivalent process group
using the same image build and the worker target/command. The worker service
owns pg-boss processing for extraction, indexing, and ai-prep; the API service
must remain enqueue-only unless an explicit legacy `*_WORKER_ENABLED` override
is approved for a break-glass local run.

Release evidence for the worker service should bind to `MON-H5-002` and
`MON-H5-003` without recording provider account IDs, task ARNs, raw object keys,
document names, tenant IDs, or customer-specific queue payloads.
