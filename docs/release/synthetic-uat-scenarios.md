...
| UAT-004 | Upload document, create new version, preview, download with reason, then soft-delete.  | Original file object is not overwritten; hold flag blocks delete; DOCUMENT\_\* audit refs exist.   | EV-UAT-004   |
...
## Desktop PWA UAT Addendum

These desktop checks extend the current synthetic-only launch scope. They do not
authorize offline document access, local search indexes, external model calls,
native sharing, or real customer documents.

| Desktop UAT     | Primary Path                                                                                         | Negative / Invariant Check                                                                                                                         | Evidence Ref   |
| --------------- | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| DESKTOP-UAT-001 | Install/open AMIC Vault from the approved browser PWA surface and launch to `/dashboard?source=pwa`. | Unauthenticated installed-app launch redirects to `/login` and does not render tenant or matter content from local cache.                          | EV-DESKTOP-003 |
| DESKTOP-UAT-002 | Log in from the installed app with approved synthetic credentials.                                   | Session handling remains server-authoritative; cookies/tokens are not recorded in evidence and are not exposed to app logs.                        | EV-DESKTOP-003 |
| DESKTOP-UAT-003 | View and download an authorized synthetic document through the installed app.                        | Server emits `DOCUMENT_VIEWED` and `DOCUMENT_DOWNLOADED` audit refs before response; desktop layer does not bypass AuditService.                   | EV-DESKTOP-003 |
| DESKTOP-UAT-004 | Run authorized search, then repeat as a user without matter access.                                  | Denied search returns safe denial or zero authorized results before result construction; no title, snippet, metadata, or target existence leaks.   | EV-DESKTOP-003 |
| DESKTOP-UAT-005 | Disconnect network and open the installed app/offline shell.                                         | Offline shell shows only the safe unavailable state and contains no tenant, matter, document, search, audit, AI, token, secret, or client markers. | EV-DESKTOP-003 |
