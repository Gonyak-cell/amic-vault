...
| EV-DMS-UI-004A | Upload receipt UI guard | `apps/web/src/components/document/document-upload-panel.test.tsx` ref proving upload queue receipt links to document detail and Matter file cabinet, keeps file-organization prep visible, shows duplicate-candidate counts, and avoids visible raw Matter references | Operator |  |
