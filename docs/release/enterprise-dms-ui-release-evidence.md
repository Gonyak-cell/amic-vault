...
| EV-DMS-UI-004C | Related document UI guard | `apps/web/src/components/document/document-action-center.test.tsx` refs proving document detail shows same-Matter related documents from the permission-scoped matter document list API, excludes the current document, displays safe title/status/type/updated-time fields, and avoids visible raw Matter references | Operator |  |
