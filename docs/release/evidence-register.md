...
| EV-UAT-001 | UAT | passed | Codex/Product QA | STAGE-RUNTIME-RLS-AWS-001 | UAT-001 auth and tenant-context path passed via staging smoke: live/ready health, unauthenticated dashboard redirect, synthetic login, authenticated dashboard, tenant-scoped API response, and negative tenant-settings denial. Remaining UAT rows are not accepted until separately executed. |
