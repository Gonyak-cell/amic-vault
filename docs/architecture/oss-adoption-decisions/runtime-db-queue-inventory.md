...
The proposed text baseline (`Pool=43`, `PgBoss=19`) is superseded by the
TypeScript-AST result: the installed root TypeScript dependency resolves
aliases, namespace imports, and locally-bound dynamic imports while excluding
comments, strings, and type-only imports. Its immutable inventory digest is
...
| `DBR` | 13 | remaining records, DLP, enterprise, AI and scheduler services |
