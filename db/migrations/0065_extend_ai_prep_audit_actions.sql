...
-- audit_events is append-only. After LAI prep audit rows have been recorded,
-- rollback cannot safely remove the AI_PREP_* actions from the allow-list.
ALTER TABLE audit_events
...
ALTER TABLE audit_events
  DROP CONSTRAINT IF EXISTS audit_events_metadata_json_safe_keys_check;

ALTER TABLE audit_events
  ADD CONSTRAINT audit_events_metadata_json_safe_keys_check CHECK (
    jsonb_typeof(metadata_json) = 'object'
    AND NOT (metadata_json ?| ARRAY[
      'body',
      'content',
      'text',
      'snippet',
...
COMMENT ON COLUMN audit_events.metadata_json IS
  'Whitelist only: reference IDs, hashes, code values, booleans, and bounded numeric values. No body/content/text/snippet/raw/prompt/response/password/token values.';
