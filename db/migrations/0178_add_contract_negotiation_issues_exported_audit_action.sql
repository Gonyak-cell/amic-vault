...
BEGIN
  ...
  FROM (
    ...
    UNION
    SELECT 'CONTRACT_NEGOTIATION_ISSUES_EXPORTED'
  ) actions;
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM audit_events
    WHERE action = 'CONTRACT_NEGOTIATION_ISSUES_EXPORTED'
    LIMIT 1
  ) THEN
    RAISE EXCEPTION 'Cannot remove CONTRACT_NEGOTIATION_ISSUES_EXPORTED from audit_events_action_check while append-only audit rows exist';
  END IF;
  ...
  FROM (
    ...
    WHERE c.conrelid = 'audit_events'::regclass
      AND c.conname = 'audit_events_action_check'
      AND match[1] <> 'CONTRACT_NEGOTIATION_ISSUES_EXPORTED'
  ) actions;
