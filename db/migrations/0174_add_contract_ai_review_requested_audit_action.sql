...
BEGIN
  ...
  FROM (
    ...
    UNION
    SELECT 'CONTRACT_AI_REVIEW_REQUESTED'
  ) actions;
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM audit_events
    WHERE action = 'CONTRACT_AI_REVIEW_REQUESTED'
    LIMIT 1
  ) THEN
    RAISE EXCEPTION 'Cannot remove CONTRACT_AI_REVIEW_REQUESTED from audit_events_action_check while append-only audit rows exist';
  END IF;
  ...
  FROM (
    ...
    WHERE c.conrelid = 'audit_events'::regclass
      AND c.conname = 'audit_events_action_check'
      AND match[1] <> 'CONTRACT_AI_REVIEW_REQUESTED'
  ) actions;
