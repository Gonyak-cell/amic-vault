...
BEGIN
  ...
  FROM (
    ...
    UNION
    SELECT 'DOCUMENT_STATUS_CHANGED'
  ) actions;
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM audit_events
    WHERE action = 'DOCUMENT_STATUS_CHANGED'
    LIMIT 1
  ) THEN
    RAISE EXCEPTION 'Cannot remove DOCUMENT_STATUS_CHANGED from audit_events_action_check while append-only audit rows exist';
  END IF;
  ...
  FROM (
    ...
    WHERE c.conrelid = 'audit_events'::regclass
      AND c.conname = 'audit_events_action_check'
      AND match[1] <> 'DOCUMENT_STATUS_CHANGED'
  ) actions;
