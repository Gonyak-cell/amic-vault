...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM audit_events
    WHERE action = 'EMAIL_RAW_DOWNLOADED'
    LIMIT 1
  ) THEN
    RAISE EXCEPTION 'Cannot remove EMAIL_RAW_DOWNLOADED from audit_events_action_check while append-only audit rows exist';
  END IF;
  ...
  FROM (
    ...
    WHERE c.conrelid = 'audit_events'::regclass
      AND c.conname = 'audit_events_action_check'
      AND match[1] <> 'EMAIL_RAW_DOWNLOADED'
  ) actions;
