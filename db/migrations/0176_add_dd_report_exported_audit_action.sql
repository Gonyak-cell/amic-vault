...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM audit_events
    WHERE action = 'DD_REPORT_EXPORTED'
    LIMIT 1
  ) THEN
    RAISE EXCEPTION 'Cannot remove DD_REPORT_EXPORTED from audit_events_action_check while append-only audit rows exist';
  END IF;
  ...
  FROM (
    ...
    WHERE c.conrelid = 'audit_events'::regclass
      AND c.conname = 'audit_events_action_check'
      AND match[1] <> 'DD_REPORT_EXPORTED'
  ) actions;
