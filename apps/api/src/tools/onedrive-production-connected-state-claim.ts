...
export interface OneDriveConnectedStateClaimCliArgs {
  dryRun: boolean;
  execute: boolean;
  runId: string;
  tenantSlug: string;
...
interface ConnectedStateVerificationReceipt {
  receipt_type?: unknown;
  status?: unknown;
  onedrive_connected_state_verified?: unknown;
  production_source_of_truth_cutover_executed?: unknown;
  verification_scope?: {
    production_surface_checked?: unknown;
    provider_connection_checked?: unknown;
  };
  prohibited_claims?: {
    office_open_save_sync_claim?: unknown;
...
interface ConnectedStateCounts {
...
interface ConnectedStatePlan {
  tenantId: string;
  actorUserId: string;
  actorRole: string;
  cutoverId: string;
  controlConstraintBlocksTrue: boolean;
  counts: ConnectedStateCounts;
  verification: ConnectedStateVerificationReceipt | null;
...
interface ConnectedStateClaimDb {
  plan(args: OneDriveConnectedStateClaimCliArgs): Promise<ConnectedStatePlan>;
  execute(
    args: OneDriveConnectedStateClaimCliArgs,
    plan: ConnectedStatePlan,
    receiptHash: string,
...
export function usage(): string {
  return [
    'usage: pnpm onedrive:production-connected-state-claim -- --dry-run|--execute --run-id <id> --tenant-slug <slug> --actor-email <email> --approval-ref <ref> --control-ref <ref> --sanitized-out <out.json> [--verification-receipt <connected-state-verification.json>]',
    '',
    'Records the approved production OneDrive connected-state claim after source-of-truth cutover and connected-state verification pass.',
    'It does not claim Office open/save/sync, Gemma indexing execution, or customer-wide go-live.',
  ].join('\n');
}

export function parseOneDriveConnectedStateClaimArgs(
  argv: readonly string[],
  env: NodeJS.ProcessEnv = process.env,
): OneDriveConnectedStateClaimCliArgs {
...
export async function runOneDriveConnectedStateClaim(
  args: OneDriveConnectedStateClaimCliArgs,
  db: ConnectedStateClaimDb = new PgConnectedStateClaimDb(args.databaseUrl),
) {
  ...
  const report = {
    ...
    verification: {
      ...
      provider_connection_checked:
        plan.verification?.verification_scope?.provider_connection_checked === true,
    },
    blockers,
    acceptance_checks: {
      source_of_truth_cutover_executed: plan.counts.sourceCutoverRows > 0,
      connected_state_not_already_claimed: plan.counts.connectedStateClaimedRows === 0,
      office_sync_not_claimed_by_this_lane: plan.counts.officeSyncClaimedRows === 0,
      ...
      connected_state_verification_receipt_pass: verificationPass,
    ...
    sanitization:
      'Receipt stores counts, hashes, safe refs, and reason codes only. Raw paths, document names, matter codes, client names, document contents, OCR excerpts, object keys, tokens, secrets, tenant-private raw labels, provider IDs, Graph payloads, and connected account identifiers are omitted.',
  };
  await writeJson(args.sanitizedOut, report);
  return report;
}

class PgConnectedStateClaimDb implements ConnectedStateClaimDb {
  constructor(private readonly databaseUrl: string) {}

  async plan(args: OneDriveConnectedStateClaimCliArgs): Promise<ConnectedStatePlan> {
    ...
    try {
      ...
      const updateResult = await client.query<{ cutover_id: string }>(
        `
          ...
          SET onedrive_connected_state_claimed = true
          WHERE tenant_id = $1
            AND cutover_id = $2
            AND status = 'executed'
            AND vault_source_of_truth = true
            AND onedrive_connected_state_claimed = false
      ...
      if (updatedCutoverRows !== 1) {
        throw new Error('ONEDRIVE_CONNECTED_STATE_UPDATE_COUNT_MISMATCH');
      }
      const auditResult = await client.query<{ event_id: string }>(
        `
          ...
          VALUES (
            ...
            'onedrive_connected_state', $3, NULL, 'success', $4::jsonb, NULL, 'PERMANENT'
          )
          RETURNING event_id
        `,
        [
          ...
          JSON.stringify({
            ...
            verification_ref: plan.verificationRef,
            run_id: args.runId,
            status_after: 'production_onedrive_connected_state_claimed',
            reason_code: 'source_cutover_and_connected_state_verification_pass',
          }),
        ],
      );
      const auditEventId = auditResult.rows[0]?.event_id;
      if (!auditEventId) throw new Error('ONEDRIVE_CONNECTED_STATE_AUDIT_INSERT_RETURNED_NO_ROW');
...
async function summarizeCounts(client: Client, tenantId: string): Promise<ConnectedStateCounts> {
    ...
    `
      SELECT EXISTS (
        ...
        WHERE conrelid = 'onedrive_source_cutovers'::regclass
          AND contype = 'c'
          AND pg_get_constraintdef(oid) ILIKE '%onedrive_connected_state_claimed = false%'
      ) AS blocks_true
    `,
  );
...
): Promise<{ receipt: ConnectedStateVerificationReceipt | null; ref: string | null }> {
  if (!filePath) return { receipt: null, ref: null };
  const parsed = JSON.parse(await readFile(filePath, 'utf8')) as ConnectedStateVerificationReceipt;
...
function validateReadiness(
  ...
  if (plan.controlConstraintBlocksTrue) blockers.push('connected_state_control_constraint_not_migrated');
  if (plan.counts.sourceCutoverRows <= 0) blockers.push('source_of_truth_cutover_not_executed');
  if (plan.counts.connectedStateClaimedRows > 0) blockers.push('connected_state_already_claimed');
  if (plan.counts.officeSyncClaimedRows > 0) blockers.push('office_sync_already_claimed_review_required');
  if (!plan.verification) blockers.push('connected_state_verification_receipt_missing');
  if (plan.verification && !verificationReceiptPass(plan.verification)) {
    blockers.push('connected_state_verification_receipt_not_passed');
  }
  return [...new Set(blockers)];
}

function verificationReceiptPass(receipt: ConnectedStateVerificationReceipt | null): boolean {
  return (
    receipt?.receipt_type === 'onedrive_connected_state_verification' &&
    (receipt.status === 'PASS' || receipt.status === 'pass') &&
    receipt.onedrive_connected_state_verified === true &&
    receipt.production_source_of_truth_cutover_executed === true &&
    receipt.verification_scope?.production_surface_checked === true &&
    receipt.verification_scope?.provider_connection_checked === true &&
    receipt.prohibited_claims?.office_open_save_sync_claim === false &&
...
function emptyPlan(blocker: string): ConnectedStatePlan {
  return {
    tenantId: '',
    actorUserId: '',
    actorRole: '',
...
async function main(): Promise<void> {
  let args: OneDriveConnectedStateClaimCliArgs;
  try {
    args = parseOneDriveConnectedStateClaimArgs(process.argv.slice(2));
  } catch (error) {
  ...
  try {
    const report = await runOneDriveConnectedStateClaim(args);
    console.log(
      JSON.stringify({
        status: report.status,
        mode: report.mode,
        onedrive_connected_state_claimed: report.onedrive_connected_state_claimed,
    ...
    console.error(
      JSON.stringify({
        code: 'ONEDRIVE_CONNECTED_STATE_CLAIM_FAILED',
        message: error instanceof Error ? error.message : 'ONEDRIVE_CONNECTED_STATE_CLAIM_FAILED',
      }),
    );
    process.exitCode = 1;
  }
