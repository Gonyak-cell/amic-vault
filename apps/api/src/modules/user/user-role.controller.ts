...
import { assignUserRoleSchema } from './dto/assign-role.dto';
...
export class UserRoleController {
  constructor(@Inject(UserRoleService) private readonly userRoleService: UserRoleService) {}
  ...
  assignRole(
    ...
    const input = parseOrValidation(() => assignUserRoleSchema.parse(body));
    return this.userRoleService.assignRole(session.tenantId, session.userId, parseUuid(userId), input);
  }
}
