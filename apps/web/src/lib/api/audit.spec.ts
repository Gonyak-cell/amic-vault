...
describe('audit API client', () => {
  ...
  it('loads the tenant audit anchor verification badge status', async () => {
    const fetchMock = vi.fn(
      async () =>
        new Response(JSON.stringify({ status: 'verified', latest: null, items: [] }), {
          status: 200,
        }),
    );
    vi.stubGlobal('fetch', fetchMock);

    await expect(getAuditAnchorStatus()).resolves.toEqual({
      status: 'verified',
      latest: null,
      items: [],
    });
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'http://localhost:3001/v1/audit-events/anchors',
      expect.objectContaining({ cache: 'no-store', credentials: 'include' }),
    );
  });
});
