...
describe('EnterpriseHardeningClient', () => {
  it('renders admin settings sections without sample defaults or raw reference inputs', () => {
    ...
    expect(html).toContain('통합 로그인');
    expect(html).toContain('추가 인증');
    expect(html).toContain('고객 관리 키');
    expect(html).toContain('보안 관제');
    expect(html).toContain('백업');
    expect(html).toContain('규정 준수');
    expect(html).toContain('문서 관리 설정');
    expect(html).toContain('문서 분류 체계');
    expect(html).toContain('Matter 템플릿');
    expect(html).toContain('검색 조건');
    expect(html).toContain('분류 체계 저장');
    ...
    expect(html).toContain('승인된 문서 세트만 Matter 화면에 표시');
    expect(html).toContain('검색 색인 운영');
    expect(html).toContain('검색 상태');
    expect(html).toContain('검색 색인, 추출/OCR, 검색 기록 집계');
