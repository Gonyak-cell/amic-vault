...
export const localAiEvalReportSchema = z
  .object({
    ...
    goldenCaseCount: z.number().int().min(0),
    claimRecall: z.number().min(0).max(1),
    claimPrecision: z.number().min(0).max(1),
    citationDocumentJaccard: z.number().min(0).max(1),
    goldenSetPass: z.boolean(),
    ...
    artifactKindMetrics: z.array(localAiEvalArtifactKindMetricSchema).max(14),
    technicalPass: z.boolean(),
