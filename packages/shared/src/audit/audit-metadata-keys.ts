export const auditMetadataKeys = [
  'before_ref',
  'after_ref',
  'reason_code',
  'role_before',
  'role_after',
  'diff_keys',
  'wall_id',
  'request_id',
  'requester_user_id',
  'approver_user_id',
  'revoked_by_user_id',
  'target_user_id',
  'approval_count',
  'expires_at',
  'member_user_id',
  'client_id',
  'matter_id',
  'item_code',
  'pending_count',
  'passed_count',
  'waived_count',
  'pending_items',
  'related_matter_id',
  'relation_type',
  'party_id',
  'conflict_check_id',
  'correlation_id',
  'ip_address',
  'method',
  'document_id',
  'file_object_id',
  'version_id',
  'base_version_id',
  'edit_session_id',
  'subversion_id',
  'subversion_no',
  'promoted_version_id',
  'subversion_reviewer_id',
  'subversion_review_id',
  'review_decision',
  'visibility_scope',
  'client_kind',
  'hash',
  'extraction_status',
  'extraction_method',
  'confidence',
  'channel',
  'scope_type',
  'scope_id',
  'identity_type',
  'batch_size',
  'candidate_version_count',
  'enqueued_job_count',
  'queue_name',
  'dead_letter_queue',
  'query_hash',
  'query_length',
  'filter_refs',
  'result_count',
  'duration_ms',
  'search_mode',
  'zero_result',
  'export_format',
  'policy_id',
  'ai_session_id',
  'model_route',
  'decision_ref',
  'document_count',
  'chunk_id',
  'included_count',
  'excluded_count',
  'included_chunk_ids',
  'excluded_chunk_ids',
  'response_length',
  'response_token_count',
  'ai_response_status',
  'escalation_required',
  'blocked_reason',
  'ai_prep_artifact_id',
  'ai_prep_status',
  'ai_prep_kind',
  'source_chunk_count',
  'generation_result',
  'fallback_reason_code',
  'prompt_hash',
  'response_hash',
  'risk_flag',
  'dlp_finding_count',
  'dlp_alert_id',
  'dlp_assessment_id',
  'dlp_scan_state',
  'dlp_policy_version',
  'dlp_restricted_finding_count',
  'dlp_requires_review',
  'dlp_review_id',
  'download_byte_count',
  'threshold_event_count',
  'threshold_byte_count',
  'window_start',
  'window_end',
  'stale_reason',
  'dead_letter_id',
  'feedback_id',
  'feedback_kind',
  'feedback_reason_code',
  'rating',
  'helpful',
  'correction_type',
  'error_types',
  'edit_distance',
  'sync_run_id',
  'node_count',
  'edge_count',
  'stale_count',
  'drift_count',
  'graph_scope',
  'consistency_status',
  'contract_type',
  'classifier_version',
  'clause_count',
  'term_count',
  'redline_change_count',
  'parser_status',
  'playbook_rule_id',
  'redline_change_id',
  'rule_id',
  'negotiation_position_id',
  'finding_hash',
  'rule_key',
  'rule_version',
  'rule_finding_count',
  'unsupported_rule_count',
  'rfi_id',
  'rfi_count',
  'mapping_id',
  'mapping_count',
  'issue_id',
  'issue_count',
  'risk_id',
  'risk_count',
  'trace_count',
  'status_before',
  'status_after',
  'severity',
  'priority',
  'mapping_status',
  'evidence_id',
  'evidence_count',
  'fact_id',
  'fact_count',
  'issue_node_id',
  'issue_node_count',
  'pleading_id',
  'pleading_count',
  'case_map_count',
  'custody_status',
  'admitted_status',
  'filing_status',
  'evidence_type',
  'pleading_type',
  'external_workspace_id',
  'external_user_id',
  'external_link_id',
  'link_count',
  'access_status',
  'nda_version',
  'watermark_ref',
  'closing_binder_id',
  'manifest_hash',
  'archive_count',
  'item_count',
  'retention_policy_id',
  'legal_hold_id',
  'archive_id',
  'disposal_request_id',
  'work_item_ref',
  'work_kind',
  'assignee_scope',
  'knowledge_candidate_id',
  'candidate_type',
  'page_kind',
  'certificate_id',
  'certificate_hash',
  'retention_days',
  'deleted_row_count',
  'storage_object_count',
  'executor_user_id',
  'provider_id',
  'provider_key',
  'metadata_hash',
  'certificate_fingerprint',
  'key_reference_id',
  'key_ref_hash',
  'key_fingerprint',
  'siem_export_id',
  'sink_type',
  'endpoint_hash',
  'seq_start',
  'seq_end',
  'event_count',
  'backup_snapshot_id',
  'table_count',
  'row_counts_hash',
  'drill_id',
  'drill_evidence_ref',
  'drill_manifest_hash',
  'schema_hash',
  'restored_schema_hash',
  'row_counts_drift_hash',
  'framework',
  'control_id',
  'evidence_ref',
  'evidence_hash',
  'readiness_status',
  'taxonomy_id',
  'document_type_code',
  'subtype_count',
  'metadata_field_count',
  'version_no',
  'template_ref',
  'template_id',
  'matter_type',
  'document_set_count',
  'refiner_id',
  'field_key',
  'performance_run_id',
  'scenario',
  'p50_ms',
  'p95_ms',
  'p99_ms',
  'target_p95_ms',
  'measurement_hash',
  'cost_snapshot_id',
  'cost_scope',
  'unit_count',
  'estimated_cost_cents',
  'cost_model_hash',
  'eval_run_id',
  'suite',
  'case_count',
  'pass_count',
  'fail_count',
  'metric_hash',
  'migration_drill_id',
  'schema_hash_before',
  'schema_hash_after',
  'learning_event_id',
  'pattern_code',
  'resolution_ref',
  'ai_gate_review_id',
  'candidate_route',
  'external_model_allowed',
  'control_hash',
  'mailbox_fingerprint_hash',
  'mailbox_binding_id',
  'addin_session_id',
  'acquisition_id',
  'message_hash',
  'email_record_id',
  'suggested_matter_id',
  'selected_matter_id',
  'confidence_band',
  'confidence_score',
  'feedback_action',
  'attachment_count',
  'attachment_id_hash',
  'filed_attachment_count',
  'idempotency_hash',
  'client_request_hash',
  'scope_count',
  'scope_set_hash',
  'outlook_status',
  'request_kind',
  'policy_decision',
  'policy_mode',
  'matter_source_mode',
  'matter_source_revision',
  'matter_source_updated_at',
  'warning_count',
  'warning_codes',
  'folder_ref_hash',
  'folder_path_hash',
  'auto_file_enabled',
  'approval_scope',
  'job_id',
  'dedupe_hash',
  'retry_count',
] as const;

export type AuditMetadataKey = (typeof auditMetadataKeys)[number];
export type AuditMetadataValue = string | number | boolean | null | readonly string[];
export type AuditMetadata = Partial<Record<AuditMetadataKey, AuditMetadataValue>>;

export function isAuditMetadataKey(value: string): value is AuditMetadataKey {
  return (auditMetadataKeys as readonly string[]).includes(value);
}
